...
<title>Orivo Health — Investor Pitch Deck</title>
...
:root{
  ...
  --coral:#FF6B6B;--coral-lo:rgba(255,107,107,.08);--coral-b:rgba(255,107,107,.2);
  --vera:#7C6FCD;--vera-lo:rgba(124,111,205,.08);--vera-b:rgba(124,111,205,.22);
...
.topbar{
  ...
  flex-shrink:0;backdrop-filter:blur(10px);z-index:10;
}
.brand{font-family:var(--serif);font-size:20px;color:var(--teal);letter-spacing:-.4px;display:flex;align-items:baseline;gap:8px}
.brand-sub{font-family:var(--sans);font-size:9px;letter-spacing:3px;text-transform:uppercase;color:var(--w3)}
.nav-r{display:flex;align-items:center;gap:10px}
.dots{display:flex;gap:5px;align-items:center}
.dot{width:5px;height:5px;border-radius:50%;background:rgba(240,242,255,.15);cursor:pointer;transition:all .3s ease;border:none;padding:0}
...
.nbtn{background:none;border:1px solid rgba(240,242,255,.1);color:var(--w2);width:30px;height:30px;border-radius:7px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s;font-size:13px;font-family:var(--sans)}
...
.slide{
  position:absolute;inset:0;padding:50px 60px;
  display:flex;flex-direction:column;
  opacity:0;pointer-events:none;
  transition:opacity .4s ease,transform .4s cubic-bezier(.4,0,.2,1);
  transform:translateY(14px);overflow-y:auto;overflow-x:hidden;
}
...
.s4{background:radial-gradient(ellipse at 50% 50%,rgba(255,107,107,.04) 0%,transparent 65%),radial-gradient(ellipse at 80% 20%,rgba(0,200,150,.04) 0%,transparent 50%),linear-gradient(160deg,#070F1C 0%,#050B18 100%)}
.s5{background:radial-gradient(ellipse at 40% 70%,rgba(74,158,255,.05) 0%,transparent 60%),linear-gradient(160deg,#060E1A 0%,#040C16 100%)}
.s6{background:radial-gradient(ellipse at 65% 45%,rgba(0,200,150,.06) 0%,transparent 60%),linear-gradient(160deg,#060E1A 0%,#040C16 100%)}
.s7{background:radial-gradient(ellipse at 35% 55%,rgba(167,139,250,.05) 0%,transparent 60%),linear-gradient(160deg,#070E1C 0%,#050B17 100%)}
.s8{background:radial-gradient(ellipse at 55% 35%,rgba(245,166,35,.05) 0%,transparent 60%),linear-gradient(160deg,#07101A 0%,#050C18 100%)}
.s9{background:radial-gradient(ellipse at 50% 50%,rgba(0,200,150,.08) 0%,transparent 60%),linear-gradient(160deg,#060C18 0%,#040810 100%)}
/* ── TYPE ── */
.ey{font-size:10px;letter-spacing:4px;text-transform:uppercase;color:var(--teal);margin-bottom:14px}
.h1{font-family:var(--serif);font-size:56px;line-height:1.02;color:var(--w1);margin-bottom:18px}
...
.pill-r{background:var(--coral-lo);border-color:var(--coral-b);color:var(--coral)}
.pillrow{display:flex;gap:8px;flex-wrap:wrap;margin-top:20px}
/* ── BUTTONS ── */
.btn{display:inline-flex;align-items:center;gap:8px;background:var(--teal);color:#030C0A;padding:11px 22px;border-radius:9px;font-size:13px;font-weight:600;border:none;cursor:pointer;font-family:var(--sans);margin-top:26px;transition:all .2s;letter-spacing:.2px}
.btn:hover{background:var(--teal-d);transform:translateY(-1px)}
.btn-o{display:inline-flex;align-items:center;gap:8px;background:transparent;color:var(--teal);border:1px solid rgba(0,200,150,.28);padding:10px 20px;border-radius:9px;font-size:13px;cursor:pointer;font-family:var(--sans);margin-top:26px;margin-left:10px;transition:all .2s}
...
.grid-bg{position:absolute;inset:0;pointer-events:none;opacity:.022;background-image:linear-gradient(rgba(0,200,150,1) 1px,transparent 1px),linear-gradient(90deg,rgba(0,200,150,1) 1px,transparent 1px);background-size:60px 60px}
/* ── STAT CARDS ── */
.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:28px}
.sc{background:var(--w4);border:1px solid var(--border);border-radius:14px;padding:24px 20px;transition:all .25s}
...
.step{background:var(--w4);border:1px solid var(--border);border-radius:14px;padding:26px 22px;position:relative;overflow:hidden;transition:all .25s}
.step:hover{border-color:rgba(0,200,150,.2)}
.step.hi{border-color:rgba(0,200,150,.22);background:rgba(0,200,150,.04)}
.step::after{content:attr(data-n);position:absolute;bottom:-14px;right:10px;font-family:var(--serif);font-size:96px;color:rgba(240,242,255,.025);line-height:1;pointer-events:none}
.step-ico{width:40px;height:40px;border-radius:10px;background:rgba(0,200,150,.1);border:1px solid rgba(0,200,150,.2);display:flex;align-items:center;justify-content:center;margin-bottom:14px;font-size:18px}
...
/* ── PRODUCT CARDS ── */
.prod-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-top:26px}
.pc{background:var(--w4);border:1px solid var(--border);border-radius:12px;padding:18px 16px;transition:all .25s;cursor:default}
...
/* ── SMS PHONE ── */
.phone{width:186px;background:#0E1520;border-radius:28px;border:1.5px solid rgba(240,242,255,.1);padding:10px;flex-shrink:0}
.notch{width:50px;height:7px;background:#1A2535;border-radius:3.5px;margin:0 auto 7px}
.ptop{background:#172030;border-radius:6px 6px 0 0;padding:8px 10px;display:flex;align-items:center;gap:7px;border-bottom:1px solid rgba(255,255,255,.05)}
.pav{width:24px;height:24px;border-radius:50%;background:linear-gradient(135deg,#00C896,#0070CC);display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;flex-shrink:0}
.pname{font-size:10.5px;color:var(--w1);font-weight:500}
.pstat{font-size:8px;color:var(--teal)}
.pmsgs{background:#0A1018;border-radius:0 0 6px 6px;padding:8px 7px;height:300px;display:flex;flex-direction:column;gap:5px;overflow:hidden}
.msg{padding:7px 10px;border-radius:11px;font-size:9.5px;line-height:1.48;max-width:92%;opacity:0;transform:translateY(5px);transition:opacity .35s ease,transform .35s ease}
...
/* ── SDOH CARD ── */
.sdoh-result{background:rgba(240,242,255,.04);border:1px solid rgba(0,200,150,.14);border-radius:13px;padding:18px;opacity:0;transition:opacity .5s ease}
...
.rh{background:var(--w4);border:1px solid var(--border);border-radius:14px;padding:24px 20px;transition:all .25s}
.rh:hover{border-color:var(--border2);background:rgba(240,242,255,.09)}
.rh-badge{display:inline-flex;align-items:center;gap:6px;border:1px solid;border-radius:100px;padding:4px 12px 4px 8px;font-size:11px;margin-bottom:14px}
...
.mc{background:var(--w4);border:1px solid var(--border);border-radius:13px;padding:20px 16px;text-align:center;transition:all .25s}
...
/* ── FOUNDER ── */
.founder-card{background:rgba(240,242,255,.04);border:1px solid rgba(0,200,150,.14);border-radius:16px;padding:28px 32px;margin-top:28px}
.fc-quote{font-family:var(--serif);font-size:17px;color:var(--w1);line-height:1.6;font-style:italic;margin-bottom:18px}
...
/* ── NO-APP SLIDE SPECIFICS ── */
.phone-compare{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:28px}
.phone-col{border-radius:16px;padding:26px 24px;border:1px solid}
.phone-col.bad{border-color:var(--coral-b);background:var(--coral-lo)}
.phone-col.good{border-color:var(--teal-b);background:var(--teal-lo)}
.pc-head{font-size:11px;letter-spacing:2.5px;text-transform:uppercase;margin-bottom:16px;font-weight:600}
.pc-item{display:flex;align-items:flex-start;gap:10px;font-size:13px;color:var(--w2);margin-bottom:10px;line-height:1.5}
.pc-ico{font-size:13px;flex-shrink:0;margin-top:1px}
.big-stat-row{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:24px}
.bs{border-radius:14px;padding:22px 18px;text-align:center;border:1px solid var(--border);background:var(--w4)}
.bs-n{font-family:var(--serif);font-size:38px;color:var(--teal);line-height:1;margin-bottom:6px}
.bs-l{font-size:12px;color:var(--w3);line-height:1.45}
/* ── SITE NAV READABILITY PATCH ── */
.site-nav a,
.nav-links a,
.nav-menu a,
.navbar a,
...
.header-nav a,
.orivo-nav-home,
.orivo-nav-right a:not(.orivo-nav-book),
header nav a:not(.btn):not(.button) {
  color:rgba(240,242,255,.68)!important;
...
.desktop-nav a:hover,
.header-nav a:hover,
.orivo-nav-home:hover,
.orivo-nav-right a:not(.orivo-nav-book):hover,
header nav a:not(.btn):not(.button):hover {
  color:rgba(240,242,255,.92)!important;
}
...
.cal-overlay{position:fixed;inset:0;background:rgba(4,8,18,.85);z-index:999;display:none;align-items:center;justify-content:center;backdrop-filter:blur(8px)}
.cal-overlay.open{display:flex}
.cal-box{background:#060C18;border:1px solid var(--teal-b);border-radius:18px;padding:40px 44px;max-width:420px;width:100%;position:relative;box-shadow:0 0 60px rgba(0,200,150,.12)}
.cal-close{position:absolute;top:16px;right:16px;background:none;border:1px solid var(--border);color:var(--w3);width:28px;height:28px;border-radius:7px;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;transition:all .2s;font-family:var(--sans)}
.cal-close:hover{border-color:var(--teal);color:var(--teal)}
.cal-logo{font-family:var(--serif);font-size:18px;color:var(--teal);margin-bottom:6px}
.cal-h{font-family:var(--serif);font-size:26px;color:var(--w1);margin-bottom:8px}
.cal-sub{font-size:13px;color:var(--w3);line-height:1.6;margin-bottom:28px}
.cal-options{display:flex;flex-direction:column;gap:10px}
.cal-opt{display:flex;align-items:center;gap:14px;background:var(--w4);border:1px solid var(--border);border-radius:11px;padding:16px 18px;cursor:pointer;transition:all .2s;text-decoration:none}
.cal-opt:hover{border-color:var(--teal-b);background:var(--teal-lo)}
.cal-opt-ico{font-size:20px;flex-shrink:0}
.cal-opt-t{font-size:14px;font-weight:500;color:var(--w1);margin-bottom:2px}
.cal-opt-s{font-size:11px;color:var(--w3)}
.cal-divider{display:flex;align-items:center;gap:10px;margin:16px 0}
.cal-divider-line{flex:1;height:1px;background:var(--border)}
.cal-divider-txt{font-size:11px;color:var(--w3)}
.cal-email{display:flex;gap:8px;margin-top:4px}
.cal-email-input{flex:1;background:rgba(240,242,255,.05);border:1px solid var(--border);color:var(--w1);padding:10px 14px;border-radius:8px;font-size:13px;font-family:var(--sans);outline:none;transition:border-color .2s}
.cal-email-input:focus{border-color:var(--teal-b)}
.cal-email-btn{background:var(--teal);color:#030C0A;border:none;padding:10px 16px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;font-family:var(--sans);white-space:nowrap;transition:all .2s}
.cal-email-btn:hover{background:var(--teal-d)}
@media(max-width:700px){
  .slide{padding:28px 22px}
  .stats,.steps,.prod-grid,.rmap,.metrics,.compare,.phone-compare,.big-stat-row{grid-template-columns:1fr}
  .two{grid-template-columns:1fr}
  .h1{font-size:38px}.h2{font-size:28px}
  .phone{display:none}
  .cal-box{padding:28px 24px;margin:0 16px}
}
</style>
</head>
<body>
...
<!-- Topbar -->
<div class="topbar">
  <div class="brand">orivo <span class="brand-sub">health</span></div>
  <div class="nav-r">
    <div class="dots" id="dotsEl"></div>
    <span class="snum" id="snm">1 / 9</span>
    <button class="nbtn" onclick="prev()">&#8592;</button>
    <button class="nbtn" onclick="next()">&#8594;</button>
  </div>
</div>
<div class="pbar"><div class="pfill" id="pf" style="width:11.11%"></div></div>
...
<div class="slide s1 on" id="s1">
  ...
  <div style="max-width:620px;position:relative;z-index:1">
    ...
    <div class="h1">Epic helps providers.<br><span style="color:var(--teal)">Orivo helps<br>patients.</span></div>
    <div class="body" style="max-width:480px;margin-top:4px">An AI that listens — in any language, on any phone, regardless of insurance — finds solutions to the real roadblocks between a patient and their health. No app to download. No portal to learn. Just a text message.</div>
    <div class="pillrow">
      <span class="pill">SMS-native</span>
      <span class="pill">Any phone. Any age.</span>
    ...
    <div>
      ...
      <button class="btn-o" onclick="goTo(8)">View the ask &#8594;</button>
    </div>
  </div>
...
<div class="slide s2" id="s2">
  ...
  <div class="two" style="gap:42px">
    <div>
      ...
      <div class="founder-card">
        <div class="founder-mini"><img class="founder-mini-photo" src="assets/josiah-parve-headshot.jpg" alt="Josiah Parve"><div><div style="font-weight:700;color:var(--w1);font-size:14px">Josiah Parve</div><div class="sm">Founder and CEO, Orivo Health</div></div></div>
        ...
        <div class="fc-creds">
          ...
          <span class="pill">Bachelor's, College of Letters &amp; Science, UWM — Genetics + Psychology focus</span>
  ...
  <div class="foot">
    <div class="sm">Orivo is personal. The problem is structural. The solution is AI.</div>
...
<div class="slide s3" id="s3">
  ...
  <div style="margin-top:22px;background:rgba(255,107,107,.05);border:1px solid rgba(255,107,107,.15);border-radius:14px;padding:22px 26px">
  ...
  <div class="foot">
    ...
    <button class="btn" onclick="next()" style="margin-top:0">The insight &#8594;</button>
  </div>
</div>

<!-- ─── S4: THE NO-APP DIFFERENTIATOR ─── -->
<div class="slide s4" id="s4">
  <div class="ey">The accessibility insight</div>
  <div class="h2" style="font-size:38px">Everyone else built<br>an app. We didn't.<br><span style="color:var(--teal)">That's the point.</span></div>

  <div class="phone-compare">
    <div class="phone-col bad">
      <div class="pc-head" style="color:var(--coral)">Every other patient health app</div>
      <div class="pc-item"><span class="pc-ico">✗</span>Requires smartphone — locks out elderly, rural, low-income patients</div>
      <div class="pc-item"><span class="pc-ico">✗</span>Needs download, account creation, onboarding</div>
      <div class="pc-item"><span class="pc-ico">✗</span>Demands digital literacy the highest-risk patients don't have</div>
      <div class="pc-item"><span class="pc-ico">✗</span>Requires behavior change — a charged phone, remembered login, learned UI</div>
      <div class="pc-item"><span class="pc-ico">✗</span>Abandonment within 30 days: <strong style="color:var(--w1)">~80%</strong> of health apps</div>
      <div class="pc-item"><span class="pc-ico">✗</span>Available to patients who are already digitally engaged — the ones who need it least</div>
    </div>
    <div class="phone-col good">
      <div class="pc-head" style="color:var(--teal)">Orivo</div>
      <div class="pc-item"><span class="pc-ico">✓</span>Works on <strong style="color:var(--w1)">any phone</strong> — flip phone, landline-tier SMS, feature phone, smartphone</div>
      <div class="pc-item"><span class="pc-ico">✓</span>Zero download. Zero onboarding. Zero new behavior required.</div>
      <div class="pc-item"><span class="pc-ico">✓</span>Patients already know how to text — we meet them there</div>
      <div class="pc-item"><span class="pc-ico">✓</span>Works in 40+ languages without switching apps or settings</div>
      <div class="pc-item"><span class="pc-ico">✓</span>Available to every Medicare/Medicaid patient — the ones who actually need it</div>
      <div class="pc-item"><span class="pc-ico">✓</span>Same infrastructure serves a patient in rural Mississippi and rural Lagos</div>
    </div>
  </div>

  <div class="big-stat-row">
    <div class="bs">
      <div class="bs-n">97%</div>
      <div class="bs-l">of Americans over 65 can receive and send a text message</div>
    </div>
    <div class="bs">
      <div class="bs-n">41%</div>
      <div class="bs-l">of Medicare patients do not own a smartphone — but every one has SMS</div>
    </div>
    <div class="bs">
      <div class="bs-n">0</div>
      <div class="bs-l">New behaviors required from a patient to use Orivo</div>
    </div>
  </div>

  <div class="foot">
    <div class="sm" style="max-width:480px">Accessibility isn't a feature. It's the entire strategy. The patients who need Orivo most are precisely the ones every app has left behind.</div>
    <button class="btn" onclick="next()" style="margin-top:0">See it work &#8594;</button>
  </div>
</div>

<!-- ─── S5: HOW IT WORKS ─── -->
<div class="slide s5" id="s5">
  <div class="ey">How Orivo works</div>
  <div class="h2" style="font-size:34px">Listen. Translate. Connect.</div>
  <div class="body" style="max-width:560px">Three steps. Any phone. Any language. The patient finally has something in their corner — and the care team finally has the context they need to act.</div>
  <div class="steps">
    <div class="step" data-n="1">
      <div class="step-ico">💬</div>
      <div class="step-t">Patient speaks naturally</div>
      <div class="sm" style="font-size:13px;color:var(--w2);line-height:1.55">Via SMS or voice — no app, no portal, no jargon. Symptoms, questions, concerns, barriers — in their own words, in any language. Orivo asks about their insurance and loads what's available to them specifically.</div>
    </div>
    <div class="step hi" data-n="2">
      <div class="step-ico">🧠</div>
      <div class="step-t">Aevo translates, enriches &amp; connects</div>
...
<div class="slide s6" id="s6">
  ...
  <div class="body" style="font-size:14px;max-width:500px;margin-bottom:20px">The most powerful thing Orivo does isn't translate clinical language. It's connect patients to resources they didn't know they had — through their insurance, their community, and one text message.</div>
  <div class="two" style="gap:28px">
    <div>
      <div class="sm" style="font-size:11px;margin-bottom:10px">Patient channel &nbsp;<span class="pill" style="font-size:10px;padding:3px 9px;vertical-align:middle">SMS · Any phone</span></div>
      <div class="phone">
        <div class="notch"></div>
        <div class="ptop">
          <div class="pav">O</div>
          <div>
            <div class="pname">Orivo Health</div>
        ...
        <div class="pmsgs" id="smsdemo">
          ...
          <div class="msg ma" id="d4">Good news — your plan covers free rides to medical appointments. Call 1-800-977-3237 and mention your member ID. They can schedule for tomorrow morning. Want me to notify your care team?</div>
        </div>
      </div>
    </div>
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div class="sm" style="font-size:12px;color:var(--w2);padding:14px 16px;background:rgba(0,200,150,.04);border:1px solid var(--teal-b);border-radius:9px">
        This patient didn't know their insurance covered rides. Their pharmacist didn't know either. Orivo knew — because we loaded their plan data and matched it to the barrier they described in plain language.
      </div>
    </div>
  </div>

  <div class="foot" style="padding-top:18px">
    <div class="sm" style="font-size:12px;color:var(--w3)">Next: product suite</div>
    <button class="btn" onclick="next()" style="margin-top:0">The product suite &#8594;</button>
  </div>
</div>
...
<div class="slide s7" id="s7">
  ...
  <div class="prod-grid">
    <div class="pc">
      ...
      <div class="pc-desc">Bidirectional patient ↔ clinical language translation via SMS, voice, or app</div>
  ...
  <div class="compare" style="margin-top:22px">
    <div class="comp-card them">
      <div class="comp-label" style="color:var(--coral)">Patient app companies</div>
      <div class="comp-items">
        <div class="ci"><span class="ci-ico">✗</span>Built for patients who are already digitally engaged</div>
        <div class="ci"><span class="ci-ico">✗</span>Requires smartphone, download, login, behavior change</div>
        <div class="ci"><span class="ci-ico">✗</span>Abandonment rates exceed 80% within 30 days</div>
        <div class="ci"><span class="ci-ico">✗</span>Doesn't know what insurance benefits the patient has</div>
        <div class="ci"><span class="ci-ico">✗</span>Serves the easiest patients — not the highest-risk ones</div>
      </div>
    </div>
    <div class="comp-card us">
      <div class="comp-label" style="color:var(--teal)">Orivo</div>
      <div class="comp-items">
        <div class="ci"><span class="ci-ico">✓</span>Any phone — SMS, voice, or app. No download required.</div>
        <div class="ci"><span class="ci-ico">✓</span>Zero new behavior required from the patient</div>
        <div class="ci"><span class="ci-ico">✓</span>Works in 40+ languages without setup</div>
        <div class="ci"><span class="ci-ico">✓</span>Loads each patient's specific insurance benefits automatically</div>
        <div class="ci"><span class="ci-ico">✓</span>Built for Medicare/Medicaid — the highest-risk, most underserved population</div>
      </div>
    </div>
  </div>
  <div class="foot">
    <div class="sm">Orivo doesn't compete with Epic. Orivo fills the gap nobody else was willing to fill.</div>
...
<div class="slide s8" id="s8">
  ...
  <div class="body" style="max-width:560px">Each horizon is independently fundable — and each one builds the data infrastructure the next one depends on. Start H1 with the data pipeline running. Arrive at Series B with the dataset nobody else will ever have.</div>
  <div class="rmap">
    <div class="rh">
      ...
      <div class="rh-body">Prove that conversational AI — on any phone, via SMS — closes the gap between patient language and clinical action. Three health system or independent pharmacy pilots. 200+ patients each. HIPAA compliant.</div>
      <div class="rh-prods">
        <div class="rh-prod"><span class="rh-prod-dot" style="background:var(--teal)"></span>Aevo Relay (SMS + voice + app)</div>
        <div class="rh-prod"><span class="rh-prod-dot" style="background:var(--teal)"></span>Aevo Rx (medication reconciliation)</div>
        <div class="rh-prod"><span class="rh-prod-dot" style="background:var(--teal)"></span>HIPAA / SOC 2 Type I</div>
        <div class="rh-prod"><span class="rh-prod-dot" style="background:var(--teal)"></span>Epic + Cerner FHIR R4 read</div>
        <div class="rh-prod"><span class="rh-prod-dot" style="background:var(--teal)"></span>Provider + pharmacy dashboard</div>
      </div>
    ...
    <div class="rh" style="border-color:rgba(74,158,255,.15);background:rgba(74,158,255,.02)">
      ...
      <div class="rh-prods">
        <div class="rh-prod"><span class="rh-prod-dot" style="background:#4A9EFF"></span>Orivo Brief (provider dashboard)</div>
  ...
  <div class="foot">
    <div class="sm">First pilot targets: independent pharmacy owners + healthcare companies running EQUIPP programs.</div>
    <button class="btn" onclick="next()" style="margin-top:0">The ask &#8594;</button>
  </div>
</div>

<!-- ─── S9: ASK / CTA ─── -->
<div class="slide s9" id="s9">
  ...
  <div class="metrics">
    ...
    <div class="mc hi">
      <div class="mn">2.1×</div>
      <div class="ml">Medication adherence lift in early pilot — 200 patients, 90 days</div>
    </div>
    <div class="mc hi">
      <div class="mn">$2M</div>
      <div class="ml">Seed round — HIPAA compliance, 3 health system pilots, 18-month runway</div>
    </div>
  </div>
  <div style="margin-top:20px;background:var(--w4);border:1px solid var(--border);border-radius:14px;padding:22px 26px">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px">
      <div>
        ...
        <div class="sm" style="font-size:12px;color:var(--w2)">Product + engineering — Aevo MVP and HIPAA-compliant infrastructure</div>
      </div>
      <div>
      ...
      <div>
        ...
        <div class="sm" style="font-size:12px;color:var(--w2)">Pilot operations — onboarding, clinical review, EHR integrations</div>
      </div>
      <div>
  ...
  <div class="foot">
    <div>
      <div class="sm" style="font-size:12px;margin-bottom:10px">Ready to put something in the patient's corner?</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn" onclick="openCal()" style="margin-top:0">Book a conversation &#8599;</button>
        <button class="btn-o" onclick="goTo(0)" style="margin-top:0">Back to start</button>
      </div>
    </div>
    <div style="text-align:right">
      <div class="sm" style="font-size:11px;color:var(--w3)">orivo.health &nbsp;·&nbsp; hello@orivo.health</div>
...
<div class="cal-overlay" id="calOverlay" onclick="closeCal(event)">
  <div class="cal-box" onclick="event.stopPropagation()">
    ...
    <div class="cal-logo">orivo health</div>
    <div class="cal-h">Let's talk.</div>
    <div class="cal-sub">Choose how you'd like to connect with the Orivo team. All conversations are confidential.</div>
    <div class="cal-options">
      <a class="cal-opt" href="https://cal.com/josiah.parve/30min" target="_blank" rel="noopener">
        <div class="cal-opt-ico">📅</div>
        <div>
          <div class="cal-opt-t">Schedule a 30-min call</div>
          <div class="cal-opt-s">Investor conversation via Cal.com</div>
        </div>
      </a>
      <a class="cal-opt" href="https://cal.com/josiah.parve/15min" target="_blank" rel="noopener">
        <div class="cal-opt-ico">🖥️</div>
        <div>
          <div class="cal-opt-t">Book a quick product demo</div>
          <div class="cal-opt-s">15-min intro for health systems, pharmacies, and partners</div>
        </div>
      </a>
    </div>
    <div class="cal-divider">
      <div class="cal-divider-line"></div>
      <div class="cal-divider-txt">or send a note directly</div>
      <div class="cal-divider-line"></div>
    </div>
    <div class="cal-email">
      <input class="cal-email-input" type="email" placeholder="your@email.com" id="calEmailInput">
      <button class="cal-email-btn" onclick="sendEmail()">Send intro &#8594;</button>
    </div>
    <div id="calEmailConfirm" style="display:none;margin-top:10px;font-size:12px;color:var(--teal)">✓ We'll be in touch shortly.</div>
  </div>
</div>
...
function startSDoHDemo(){
  ...
  const seq=[
    {id:'d1',d:400},{id:'dt1',d:1100,ti:1},
    {id:'d2',d:2300,ht:'dt1'},{id:'d3',d:3200},
    {id:'dt2',d:4000,ti:1},{id:'d4',d:5300,ht:'dt2'}
  ];
  ...
  setTimeout(()=>document.getElementById('sdohcard').classList.add('show'),6200);
}
// Cal.com booking modal
function openCal(){
...
function sendEmail(){
  const input=document.getElementById('calEmailInput');
  const v=input.value.trim();
  if(!v||!v.includes('@')){
    input.style.borderColor='var(--coral-b)';
    return;
  }
  input.style.borderColor='var(--teal-b)';
  const subject=encodeURIComponent('Orivo Health intro request');
  const body=encodeURIComponent(`Hi Orivo Health team,

I'd like to connect about Orivo Health.

My email: ${v}

Best,`);
  window.location.href=`mailto:hello@orivo.health?subject=${subject}&body=${body}`;
  document.getElementById('calEmailConfirm').style.display='block';
}
// Touch swipe
let tx=0;
